...
# ----------------- STAGE-3 INTEGRATION (Story -> TTS -> GPU Queue -> Preview) -----------------
...
try:
    ...
    def generate_and_save_tts(text, lang="en"):
        # fallback: create a small dummy file path or raise for missing dependency
...
try:
    ...
    def add_job(payload):
        jid = str(uuid.uuid4())
        _local_queue[jid] = {"status":"queued","progress":0,"payload":payload}
        ...
        def _sim():
            ...
            _local_queue[jid]["result"] = {"video": payload.get("demo_output","/no/output")}
...
def create_video():
    ...
    try:
        tts_path = generate_and_save_tts(script, lang=lang)
    except Exception as e:
...
# This endpoint will create a tiny 4-6s demo video (if moviepy available) using TTS and a placeholder image.
...
def stage3_preview_demo(job_id):
    ...
    try:
        ...
        return jsonify({"error":"moviepy not available for preview", "details": str(e)}), 501

    # create a short demo clip using first 6 seconds of TTS audio (or silent clip)
    tts = meta.get("tts")
    duration = 6
    if tts and os.path.exists(tts):
        try:
            ...
            if not os.path.exists(img):
                ...
                imgclip = ColorClip(size=(720,1280), color=(20,20,20)).set_duration(duration)
                # overlay text
                from moviepy.video.VideoClip import TextClip
                txt = TextClip("Visora Preview", fontsize=40, color='white').set_position('center').set_duration(duration)
                demo = CompositeVideoClip([imgclip, txt]).set_duration(duration).set_audio(a)
                outp = os.path.join(OUTPUTS, f"preview_{job_id}.mp4")
                demo.write_videofile(outp, codec="libx264", audio_codec="aac", threads=0, preset="fast")
                demo.close(); imgclip.close()
                return send_file(outp, as_attachment=False)
            else:
                clip = ImageClip(img).set_duration(duration).set_audio(a)
                outp = os.path.join(OUTPUTS, f"preview_{job_id}.mp4")
                clip.write_videofile(outp, codec="libx264", audio_codec="aac", threads=0, preset="fast")
...
def preview_video():
    ...
    try:
        ...
        color_clip = ColorClip(size=(1280, 720), color=(20, 20, 20), duration=duration)

        txt = TextClip("🎬 Visora AI Preview", fontsize=60, color="white", font="Arial-Bold")
        txt = txt.set_duration(duration).set_position(("center", "center"))
        final = CompositeVideoClip([color_clip, txt]).set_audio(audio)

        preview_path = os.path.join("renders", f"preview_{uuid.uuid4().hex[:8]}.mp4")
        os.makedirs("renders", exist_ok=True)
        final.write_videofile(preview_path, codec="libx264", fps=24, audio_codec="aac", threads=2, verbose=False, logger=None)
